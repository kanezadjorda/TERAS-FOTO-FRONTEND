...
export default function BookingsPage() {
	...
	// Fetch bookings dari backend menggunakan SWR
	const {
		data: bookingsResponse,
		error,
		isLoading,
	} = useSWR(['/admin/bookings', currentPage, activeTab, searchQuery, selectedService], () =>
		getAllBookingsAdmin({
			page: currentPage,
			limit: 10,
	...
	// Filter bookings berdasarkan tab aktif, search query, dan tipe layanan (hanya untuk mockBookings)
					...
					<div className="mt-2">
						{isLoading ? (
							...
							<span className="font-poppins font-bold text-3xl text-[#705D00] block">
								{String(stats.pendingApproval).padStart(2, '0')}
							</span>
						)}
				...
				<div className="bg-white border border-gray-100 rounded-[24px] p-6 flex flex-col justify-between shadow-sm hover:shadow-md transition-all min-h-[140px]">
					...
					<div className="mt-2">
						{isLoading ? (
							...
							<span className="font-poppins font-bold text-3xl text-[#4D4732] block">
								{formatRupiah(stats.revenueForecast)}
							</span>
						)}
